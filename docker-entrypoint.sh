...
#
# Dispatch the single vgi-reghive image into one of its transports:
...
#
# The worker is stateless (regf parser compiled in; hive input arrives per
# ATTACH as a BLOB or a file glob the worker opens per query), so there is no
...
case "${1:-http}" in
  http)
    ...
    exec reghive-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec reghive-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec reghive-worker "$@"
