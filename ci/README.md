# CI: the vgi-reghive worker integration suite
...
(`test/sql/*.test`) against the vgi-reghive VGI worker through the **real DuckDB
...
| `TRANSPORT`  | `VGI_REGHIVE_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/reghive-worker`        | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `reghive-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/reghive.<pid>.sock`         | `reghive-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
1. **Build the worker** — `cargo build --release --bin reghive-worker`. The
   compiled `target/release/reghive-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_REGHIVE_WORKER}`).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-reghive `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_REGHIVE_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_REGHIVE_WORKER` accordingly, stages the
...
WORKER_BIN="$PWD/target/release/reghive-worker" \
TRANSPORT=subprocess \
  ci/run-integration.sh

# HTTP leg: launches `reghive-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/reghive-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `reghive-worker --unix /tmp/reghive.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/reghive-worker" \
