...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-reghive
...
# is what LOCATION (VGI_REGHIVE_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_REGHIVE_WORKER = the stdio worker command (DuckDB spawns it).
#   http        start `reghive-worker --http` (auto port; advertises `PORT:<n>`
#               on stdout), VGI_REGHIVE_WORKER = http://127.0.0.1:<port>.
#               If VGI_REGHIVE_WORKER is ALREADY set to an http(s):// URL (e.g. a
#               pre-launched container in the docker image_test), it is used
#               as-is and no local worker is spawned.
#   unix        start `reghive-worker --unix <sock>` (advertises `UNIX:<sock>`
#               on stdout), VGI_REGHIVE_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
#   WORKER_BIN        path to the compiled reghive-worker binary (used to launch
#                     the http/unix servers, and the stdio LOCATION). Falls back
#                     to VGI_REGHIVE_WORKER when that is a bare command (subprocess).
# Optional:
...
# Absolute path to the committed golden hive fixtures. The .test files reference
# them as '${VGI_REGHIVE_FIXTURES}/<name>.hve' (the sqllogictest runner expands
# ${ENV}); read_hive globs them and read_blob feeds the scalar/subtree probes.
export VGI_REGHIVE_FIXTURES="${VGI_REGHIVE_FIXTURES:-$REPO/tests/hives}"
...
WORKER_BIN="${WORKER_BIN:-$REPO/target/release/reghive-worker}"
...
# Bring up the out-of-band server for http/unix and resolve VGI_REGHIVE_WORKER.
...
start_server_and_set_location() {
  local kind="$1"
  : "${WORKER_BIN:?path to the reghive-worker binary (WORKER_BIN)}"
  ...
  case "$kind" in
    http)
      ...
      export VGI_REGHIVE_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_REGHIVE_SOCK:-/tmp/reghive.$$.sock}"
      ...
      export VGI_REGHIVE_WORKER="unix://$SOCK_PATH"
...
case "$TRANSPORT" in
  subprocess)
    ...
    # VGI_REGHIVE_WORKER (e.g. a bare command) if the caller set one.
    export VGI_REGHIVE_WORKER="${VGI_REGHIVE_WORKER:-$WORKER_BIN}"
    ;;
  http)
    # Honor a pre-launched HTTP worker (e.g. a running container in the docker
    # image_test): if VGI_REGHIVE_WORKER already points at an http(s) URL, use it
    # and skip spawning a local binary. The awk preprocessor still injects
    # httpfs because TRANSPORT=http.
    if [[ "${VGI_REGHIVE_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_REGHIVE_WORKER"
    else
...
: "${VGI_REGHIVE_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_REGHIVE_WORKER, pattern: $TEST_PATTERN) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_REGHIVE_WORKER." >&2
  exit 1
fi
