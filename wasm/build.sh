#!/usr/bin/env bash
# Build the browser (`worker:`) build of the reghive VGI worker: reghive-wasm
...
#     recompiles compiler_builtins with atomics. `rustup component add rust-src`
#     for that nightly, plus the wasm32-unknown-emscripten target.
set -euo pipefail
cd "$(dirname "$0")/.."
...
LIB="target/$TARGET/release/libreghive_wasm.a"
...
#
# MALLOC=mimalloc is load-bearing for multithreaded serve threads. Emscripten's
# default dlmalloc serializes every allocation on a global lock; mimalloc is
# thread-caching and scales far better under concurrent serve threads.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which this worker overflows — the parser + Arrow encoders nest deeply, and an
